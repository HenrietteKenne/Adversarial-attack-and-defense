...
def show_images(e, x, x_adv, x_fake, save_dir):
    ...
    for i in range(5):
        ...
        axes[0, i].imshow(x[i].cpu().numpy().transpose((1, 2, 0))[:,:,0], cmap="gray")
        # axes[0, i].imshow(x[i, 0].cpu().numpy(), cmap="gray")
        axes[0, i].set_title("Normal")

        axes[1, i].imshow(x_adv[i].cpu().numpy().transpose((1, 2, 0))[:,:,0], cmap="gray")
        # axes[1, i].imshow(x_adv[i, 0].cpu().numpy(), cmap="gray")
        axes[1, i].set_title("Adv_img")

        axes[2, i].imshow(x_fake[i].cpu().numpy().transpose((1, 2, 0))[:,:,0], cmap="gray")
        # axes[2, i].imshow(x_fake[i, 0].cpu().numpy(), cmap="gray")
        axes[2, i].set_title("APE")
    plt.axis("off")
